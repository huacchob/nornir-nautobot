...
class DispatcherMixin:
    ...
    def compliance_config(  # pylint: disable=too-many-positional-arguments
        ...
        if not pathlib.Path(intended_file).exists():
    ...
    def generate_config(  # pylint: disable=too-many-positional-arguments,too-many-locals
        ...
        with pathlib.Path(output_file_location).open("w", encoding="utf8") as filehandler:
    ...
    def _save_file(cls, logger, backup_file: str, _running_config: str) -> None:
        ...
        if backup_file:
            ...
            with pathlib.Path(backup_file).open("w", encoding="utf8") as filehandler:
...
class NetmikoDefault(DispatcherMixin):
    ...
    def get_git_command(
        ...
        if not pathlib.Path(command_file_path).exists():
...
class ApiDefault(DispatcherMixin, ConnectionMixin, ABC):
